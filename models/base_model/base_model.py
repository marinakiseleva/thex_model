...
from thex_data.data_consts import non_numeric_cols
...
class BaseModel(ABC):
    ...
    def run_model(self, cols=None, col_match=None, test_on_train=False, incl_redshift=False, folds=3):
        ...
        :param incl_redshift: Boolean on using redshift as a feature
        ...
        if cols is not None:
            ...
            if col_match is not None:
                ...
                col_list = []
                for c in cols:
                    if not any(drop in c for drop in non_numeric_cols):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = self.get_model_data(
            col_list, incl_redshift, test_on_train)
        self.train_model()
    ...
    def get_model_data(self, col_list, incl_redshift, test_on_train):
        X_train, X_test, y_train, y_test = get_train_test(col_list, incl_redshift)
